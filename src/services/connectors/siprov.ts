...
export class SiprovConnector implements AssociationSystemConnector {
  ...
  async getFieldDefinitions(entidade?: SyncEntity): Promise<ExternalFieldDefinition[]> {
    ...
    const fields: ExternalFieldDefinition[] = [
      ...
      },
      {
        ...
        obrigatorio: true,
        entidade: "associados",
        opcoes: ["ativo", "inadimplente", "cancelado"],
      },
      {
        campo: "cod_beneficio",
        rotulo: "Código do benefício",
        tipo: "texto",
        obrigatorio: true,
        entidade: "beneficios",
      },
      {
        campo: "descricao_beneficio",
        rotulo: "Descrição do benefício",
        tipo: "texto",
        obrigatorio: false,
        entidade: "beneficios",
      },
      {
        campo: "protocolo_ocorrencia",
        rotulo: "Protocolo da ocorrência",
        tipo: "texto",
        obrigatorio: true,
        entidade: "ocorrencias",
      },
      {
        campo: "status_ocorrencia",
        rotulo: "Status da ocorrência",
        tipo: "enumerado",
        obrigatorio: true,
        entidade: "ocorrencias",
        opcoes: ["aberta", "em_analise", "encerrada"],
      },
      {
        campo: "cod_boleto",
        rotulo: "Código do boleto",
        tipo: "texto",
        obrigatorio: true,
        entidade: "cobrancas",
      },
  ...
  async importVehicles(config: ConnectorConfig): Promise<SyncResult> {
    ...
    return buildSyncResult({
      ...
      entidade: "veiculos",
      direction: "import",
      seedKey: `${config.id}-veiculos-siprov`,
      durationMs: 700,
    });
  }
  ...
  async importFinancialSummary(config: ConnectorConfig): Promise<SyncResult> {
    ...
    return buildSyncResult({
      ...
      direction: "import",
      seedKey: `${config.id}-financeiro-siprov`,
      durationMs: 750,
    });
  }
  ...
  async exportMembership(
    ...
    return buildSyncResult({
      ...
      entidade: "adesoes",
      direction: "export",
      seedKey: `${config.id}-${request.membershipId}`,
      durationMs: 500,
    });
  }
  ...
  async syncStatus(config: ConnectorConfig, entidade: SyncEntity): Promise<SyncResult> {
    ...
    return buildSyncResult({
      ...
      entidade,
      direction: "import",
      seedKey: `${config.id}-status-${entidade}`,
      durationMs: 300,
    });
  }
