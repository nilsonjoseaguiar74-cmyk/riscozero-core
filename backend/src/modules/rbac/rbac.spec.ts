...
describe("RBAC", () => {
  ...
  it("mantém os acessos mínimos de cada perfil do protótipo", () => {
    expect(ROLE_PERMISSIONS.comercial).toEqual(expect.arrayContaining(["dashboard.view", "crm.view", "crm.edit"]));
    expect(ROLE_PERMISSIONS.gestor_trafego).toEqual(expect.arrayContaining(["traffic.view", "traffic.manage"]));
    expect(ROLE_PERMISSIONS.desenvolvedor).toEqual(expect.arrayContaining(["developer.access", "integrations.view"]));
  });
});
